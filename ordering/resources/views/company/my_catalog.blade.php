                    ...
                    <form method="post" id="frm1">
                    	...
                    	<div class="form-group">
							...
							<select id="location" name="location" class="form-control">
								...
								<option value="{{$loc['id']}}">{{$loc['address1']}}, {{$loc['city']}}</option>
								@endforeach
							</select>
						</div>
						@foreach($cat as $row)
						@if(count($row['company_products']) > 0)
						@if($i!=0)
                        <br><hr><br>
                        @endif
						...
						<table class="tablesaw table-striped table-hover table-bordered table">
							...
							<tbody>
								@foreach($row['company_products'] as $inrow)
								<tr>
									<td>
                                	...
                                	<td>
                                    	<input hidden type="text" class="input-text" name="price<?php echo $inrow['id']; ?>" id="price" value="<?php echo $inrow['price']; ?>" > $ {{ $inrow['price'] }}
                                    </td>
                                    <td>
